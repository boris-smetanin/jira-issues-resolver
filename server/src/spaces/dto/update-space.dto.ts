...
export const updateSpaceDto = z
  .object({
    ...
    allowedStatuses: z
      ...
      .min(1),
    agentLabels: z.array(noNewlines('agentLabels entries must not contain newlines')).min(1),
    targetStatusName: noNewlines('targetStatusName must not contain newlines'),
    // Slice 16b: optional npmrc auth for private packages. Both null →
    // public packages only; both non-empty → service uses them. The
    // sentinel `'<UNCHANGED>'` for npmrcEnvValue tells the service "don't
    // touch the encrypted value, just keep whatever's in the DB" — the
    // UI sends this when the user hasn't re-typed the token.
    npmrcEnvName: z
      .string()
      .trim()
      .max(64)
  ...
  .superRefine((data, ctx) => {
    // XOR validation. The sentinel '<UNCHANGED>' means "value not
    // re-typed; keep what's already in the DB" — the service interprets
    // it. If env name is set, value must be either re-typed or
    // explicitly '<UNCHANGED>'.
    const hasName = data.npmrcEnvName !== null;
    const hasValue = data.npmrcEnvValue !== null;
    if (hasName && !hasValue) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['npmrcEnvValue'],
        message: 'npmrcEnvValue is required when npmrcEnvName is set',
      });
    }
    if (!hasName && hasValue) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['npmrcEnvName'],
        message: 'npmrcEnvName is required when npmrcEnvValue is set',
      });
    }
  });
